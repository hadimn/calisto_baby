...
class SocialMedia
{
    ...
    public function updateSocialMedia($social_id, $link, $enabled, $bg_color)
    {
        $sql = "UPDATE social_media SET link = ?, enabled = ?, bg_color = ? WHERE social_id = ?";
        $stmt = $this->conn->prepare($sql);
        if ($stmt->execute([$link, $enabled, $bg_color, $social_id])) {
            return true;
        }
        return false;
    }
